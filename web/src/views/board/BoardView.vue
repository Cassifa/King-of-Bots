...
export default {
    ...
    setup() {
        ...
        for(int i=key,t=0;t<4;i=(i+1)%4,t++){//随机一个方向开始判断
    ...
    Bot3.value=`    private final int[][] now_g= new int[13][14];
    private int eval(int x,int y,int left){//这一步走哪里 剩余搜索长度
        ...
        for(int i=0;i<4;i++){
            ...
            if(ans>=6)return ans+1;//某一格能走七步直接认定为优秀方案，后续搜索剪枝
        }
        now_g[x][y]=0;
        return ans+1;
    }
...
public class Bot implements java.util.function.Supplier<Integer>{
    ...
    static class Cell{//蛇的身体块 含有x,y两个方向坐标
        public int x,y;
    ...
    public List<Cell> getCells(int sx,int sy,String steps){//获取一条完整的蛇 数组尾为蛇头
    ...
    public Integer nextMove(String input){
        ...
        int aSx=Integer.parseInt(stirs[1]),aSy=Integer.parseInt(stirs[2]);//处理自己的蛇
        int bSx=Integer.parseInt(stirs[4]),bSy=Integer.parseInt(stirs[5]);//处理对手的蛇
